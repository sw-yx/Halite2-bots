...
def getEnemyDetails(game_map, team_ships, ship):
    ...
    closest_viable_planets = sorted(
        closest_viable_planets, key=lambda k: ship.calculate_distance_between(k))
    ...
    if isinstance(a, hlt.entity.Ship) and isinstance(b, hlt.entity.Planet) and a.can_dock(b):
        ...
        if navigate_command:
            if isinstance(b, hlt.entity.Planet):
                logging.info(log + " | ship: " + str(a.id) +
                             " going to Planet " + str(b.id))
            else:
                logging.info(log + " | ship: " + str(a.id) +
                             " going to Ship " + str(b.id))
...
while True:
    ...
    if rambo < 0:  # no rambo assigned
        avail_ships = [
            x for x in team_ships if x.docking_status != x.DockingStatus.UNDOCKED]
    ...
    if rambo >= 0:  # if there is a rambo
        ...
        if ramboShip == None:
            ...
            (closest_viable_planets, closest_enemy_ships, closest_docked_enemy_ships,
             entities_by_distance) = getEnemyDetails(game_map, team_ships, ramboShip)
            if ramboTarget >= 0:  # there is a target
                ramboTargetShip = [
                    x for x in closest_enemy_ships if x.id == ramboTarget]
                ramboTargetShip = ramboTargetShip[0] if len(
                    ramboTargetShip) > 0 else closest_enemy_ships[-1]
                ramboTarget = ramboTargetShip.id
            else:  # there is no target, set the target
                ramboTargetShip = closest_docked_enemy_ships[-1] if len(
                    closest_docked_enemy_ships) > 0 else closest_enemy_ships[-1]
                ramboTarget = ramboTargetShip.id
            goto(ramboShip, ramboTargetShip, game_map,
                 command_queue, 'rambo attack')
            logging.info("rambo: id " + str(ramboShip.id) +
                         " health: " + str(ramboShip.health) + str(ramboShip))
            logging.info("ramboTarget: id " + str(ramboTargetShip.id) +
                         " health: " + str(ramboTargetShip.health))
    ...
    battlemode = len(team_ships) > 10 and health_enemyships + 255 * \
        5 < health_myships  # if we have more ships, just attack
    undockcounter = 0
    ...
    if len(allplayers) == 2 and turn < 50 and dist < 30:
        for idx, ship in enumerate(team_ships):
            ...
            if ship.docking_status == ship.DockingStatus.DOCKED:
                ...
                (closest_viable_planets, closest_enemy_ships, closest_docked_enemy_ships,
                 entities_by_distance) = getEnemyDetails(game_map, team_ships, ship)
                # target_ship = enemyships[idx % len(enemyships)]
                # and (idx == 0 or idx == 3):
                if len(closest_docked_enemy_ships) > 0:
                    target_ship = closest_docked_enemy_ships[0] if len(
                        closest_docked_enemy_ships) > 0 else closest_enemy_ships[0]
                    goto(ship, target_ship, game_map, command_queue,
                         'bumrush attack-closest')  # attack
                else:
                    # colonize
                    goto(
                        ship, closest_viable_planets[0], game_map, command_queue, 'bumrush colonize')
    else:
        # no bum rush
        for idx, ship in enumerate(team_ships):
            ...
            if len(closest_viable_planets) > 0:
                ...
                if ship.can_dock(target_planet):
                    ...
                    if target_planet in planned_planets:
                        ...
                        target_ship = closest_docked_enemy_ships[0] if len(
                            closest_docked_enemy_ships) > 0 else closest_enemy_ships[0]
                        goto(ship, target_ship, game_map,
                             command_queue, 'attack')  # attack
                    else:
                        goto(ship, target_planet, game_map, command_queue,
                             'go to planet')  # go to planet
            ...
            # FIND SHIP TO ATTACK!
                ...
                target_ship = closest_docked_enemy_ships[0] if len(
                    closest_docked_enemy_ships) > 0 else closest_enemy_ships[0]  # 10% chance of attacking furthest?
                goto(ship, target_ship, game_map, command_queue,
                     'planetsarefull, attack')  # attack
